...
# Функция для расширённой атаки мага (для Archer будет отдельная)
...
# Новая функция меню для атаки лучника (отдельная от мага)
def archer_attack_menu(attacker, source_label, current_turn):
    print("\nЛучник использует расширенную атаку (Огненный шар)!")
    targets = extended_targets(source_label)
    enemy_targets = [t for t in targets if board.get(t) is not None and board[t].side != current_turn]
    if not enemy_targets:
        print("Нет доступных расширённых целей для атаки.")
    ...
    enemy_options = [f"{board[t].tclass} на клетке {t} (H:{board[t].hp} A:{board[t].attack} D:{board[t].defense})" for t in enemy_targets]
    target1_choice = choose_option("Выберите первую цель для атаки: ", enemy_options)
    target1 = next((t for t, opt in zip(enemy_targets, enemy_options) if opt == target1_choice), None)
    result = combat(attacker, board[target1], allow_counterattack=False)
    if result == "defender_dead":
        board[target1] = None
    elif result == "attacker_dead":
        board[source_label] = None
    ...
    enemy_targets = [t for t in enemy_targets if t != target1 and board.get(t) is not None and board[t].side != current_turn]
    if enemy_targets:
        enemy_options2 = [f"{board[t].tclass} на клетке {t} (H:{board[t].hp} A:{board[t].attack} D:{board[t].defense})" for t in enemy_targets]
        target2_choice = choose_option("Выберите вторую цель для атаки: ", enemy_options2)
        target2 = next((t for t, opt in zip(enemy_targets, enemy_options2) if opt == target2_choice), None)
        result = combat(attacker, board[target2], allow_counterattack=False)
        if result == "defender_dead":
            board[target2] = None
        elif result == "attacker_dead":
            board[source_label] = None

# Функция специального удара для рыцаря (2x damage)
...
# Функция специального отбрасывающего удара для солдата
...
# Функция применения урона с учётом щита
def apply_damage(target, damage):
    if damage <= target.defense:
        reduction = damage // 2
        target.defense = max(target.defense - reduction, 0)
...
# Функция ремонта щита для солдата (25% от max_defense)
...
# Функция применения урона с учетом щита
def apply_damage(target, damage):
    if damage <= target.defense:
        reduction = damage // 2
        target.defense = max(target.defense - reduction, 0)
...
# Функция стандартного combat с учетом щита
def combat(attacker, defender, allow_counterattack=True):
    raw_dmg = attacker.attack - defender.defense
    if raw_dmg > 0:
        dmg = raw_dmg
